...
//matrisleri alt matrislere bolme islemi
    for (int i = 0; i < new_row; i++){
        for(int j = 0; j < new_col; j++){
            a[i][j] = matrix1[i][j];
            b[i][j] = matrix1[i][j + new_col];
            c[i][j] = matrix1[i + new_row][j];
            d[i][j] = matrix1[i + new_row][ + new_col];
            e[i][j] = matrix2[i][j];
...
// 4 kucuk matrisi birlestirerek sonucu olusturur
    for (int i = 0; i < new_row; i++){
        for (int j = 0; j < new_col; j++){
            result[i][j] = c11[i][j];
